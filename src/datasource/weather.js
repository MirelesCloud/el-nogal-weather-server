...
class IrriSatAPI extends RESTDataSource {
  ...
  async getMapsDates() {
    ...
    return Array.isArray(response.items) ? response.items.slice(0,10).map(response => this.mapsDatesReducer(response)) : []
  }
  ...
  async getMapsLayer(data) {
    
    data = "2020-05-08"
    const response = await this.get(`maps/layers/${data}`)
...
class AgroAPI extends RESTDataSource {
  ...
  async getNDVI() {
    ...
    return Array.isArray(response) ? response.reverse().map(response => this.ndviReducer(response)) : []
  }
